...
class InternVideo2(nn.Module):
    ...
    def _forward_features(self, x):
        print("x shape0", x.shape) #x shape1 torch.Size([1, 8, 256, 1408])
        x = self.patch_embed(x.type(self.dtype))
        B, T, L, C = x.shape  # T: temporal; L: spatial
        print("x shape1", x.shape) #x shape1 torch.Size([1, 8, 256, 1408])

        x = x.view([B, T * L, C])
        print("x shape2", x.shape) # x shape1 torch.Size([1, 2048, 1408])

        # append cls token
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        print("x shape3", x.shape) # x shape1 torch.Size([1, 2049, 1408])
        ...
        if self.sep_pos_embed:
            ...
            pos_embed = self.pos_embed
        print("x shape4", x.shape)  # x shape3 torch.Size([1, 2049, 1408])
        print("pos_embed shape4", pos_embed.shape)  # x shape3 torch.Size([1, 2049, 1408])
        ...
        #print("model_output shape", x.shape)
        x = x[:, 1:, :]
        
        # print("x shape4", x.shape) # x shape4 torch.Size([1, 2048, 1408])
        
        x_reshaped = x.reshape(B, T, L, C)
        #print("Reshaped x:", x_reshaped.shape)  # x shape4 torch.Size([1, 8, 256, 1408])
        
        # Mean pooling along the 256-axis (dim=2)
        x_pooled = x_reshaped.mean(dim=2) 
        #print("After mean pooling:", x_pooled.shape)  # [1, 8, 1408]

        x_pooled = self.feature_norm(x_pooled)
        
        #print("model_output shape", x_pooled.shape) # [1, 8, 1408]
