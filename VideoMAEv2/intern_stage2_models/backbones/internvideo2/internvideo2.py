...
class PretrainInternVideo2(nn.Module):
    ...
    # @torch.cuda.amp.autocast(enabled=False)
    def forward(self, x, mask=None, use_image=False, x_vis_return_idx=-1, x_vis_only=True):
    ...
    def _forward_feature(self, x, mask=None, use_image=False, x_vis_return_idx=-1, x_vis_only=True):
        #print("x shape0", x.shape) #  x shape0 torch.Size([28, 3, 4, 224, 224])
        ...
        #print("x shape1", x.shape) # x x shape1 torch.Size([28, 4, 256, 1408])

        
        x = x.view([B, T * L, C])
        #print("x shape2", x.shape) # x shape2 torch.Size([28, 1024, 1408])

        # append cls token
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)

        #print("x shape3", x.shape)  x shape3 torch.Size([28, 1025, 1408])
        ...
        #print("x shape4", x.shape) # x shape4 torch.Size([28, 1025, 1408])
        #print("pos_embed shape4", pos_embed.shape) # pos_embed shape4 torch.Size([1, 1025, 1408])
        ...
        #print("x3", x.shape) # x3 torch.Size([28, 1025, 1408])

        x = x[:, 1:, :]
        
        #print("x shape4", x.shape) # x shape4 torch.Size([1, 2048, 1408]) x shape4 torch.Size([28, 1024, 1408])

        
        x_reshaped = x.reshape(B, T, L, C)
        #print("Reshaped x:", x_reshaped.shape)  # Reshaped x: torch.Size([28, 4, 256, 1408])

        # Mean pooling along the 256-axis (dim=2)
        x_pooled = x_reshaped.mean(dim=2) 
        #print("After mean pooling:", x_pooled.shape)  # After mean pooling: torch.Size([28, 4, 1408])

        

        x_pooled = self.feature_norm(x_pooled)
        
        #print("model_output shape", x_pooled.shape) # model_output shape torch.Size([28, 4, 1408])
